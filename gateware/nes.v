...
wire reset = bootvector_flag || reset_nes;
...
assign cart_ce = (cart_pre & ppu_ce);  // First PPU cycle where cpu data is visible.

// Signals
wire cart_pre  = (ppu_tick_count == (cpu_tick_count[2] ? 1 : 0));
wire ppu_read  = (ppu_tick_count == (cpu_tick_count[2] ? 2 : 1));
wire ppu_write = (ppu_tick_count == (cpu_tick_count[2] ? 1 : 0));
...
reg [1:0] ppu_tick_count = 0;

reg [1:0] last_sys_type;
reg [2:0] cpu_tick_count;

wire skip_ppu_cycle = (cpu_tick_count == 4) && (ppu_tick_count == 0);

always @(posedge clk) begin
    // Realign if the system type changes.
  last_sys_type <= sys_type;

  if (reset_nes || (last_sys_type != sys_type)) begin
    bootvector_flag <= 1'b1;
    odd_or_even     <= 1'b1;
    ...
    if (~freeze_clocks | ~(div_ppu == (div_ppu_n - 1))) begin
      if (~skip_ppu_cycle)
        div_cpu <= cpu_ce || (ppu_ce && div_cpu > div_cpu_n) ? 1 : div_cpu + 1;

      div_ppu <= ppu_ce ? 1 : div_ppu + 1;

      // reset the ticker on the first ppu tick at or after a cpu tick.
      if (cpu_ce)
        ppu_tick_count <= 0;
      else if (ppu_ce)
        ppu_tick_count <= ppu_tick_count + 1;
    end

    // Add one extra PPU tick every 5 cpu cycles for PAL.
    if (cpu_ce && sys_type[0])
      cpu_tick_count <= cpu_tick_count[2] ? 3'd0 : cpu_tick_count + 1;
    ...
    if (|faux_pixel_cnt)
      faux_pixel_cnt <= faux_pixel_cnt - 1;

    if (skip_pixel && (faux_pixel_cnt == 0)) begin
      freeze_clocks <= 1'b1;
      faux_pixel_cnt <= {div_ppu_n - 3'd1, 1'b0} + 1;
    end

    if (cpu_ce) begin
       odd_or_even     <= ~odd_or_even;
       bootvector_flag <= 1'b0;
    end
  end
end
...
assign save_written = (mapper_flags[7:0] == 8'h14) ? (prg_linaddr[21:18] == 4'b1111 && prg_write) : (prg_addr[15:13] == 3'b011 && prg_write);
...
cart_top multi_mapper (
  ...
  .ce                (cart_ce && enable && ~reset),  // M2 (held in high impedance during reset)
...
assign cpumem_read  = (prg_read & prg_allow) | (prg_write && prg_conflict) && !reset_nes;
assign cpumem_write = prg_write && prg_allow && !reset_nes;
assign cpumem_dout  = prg_din;
assign ppumem_addr  = chr_linaddr;
assign ppumem_read  = chr_read && !reset_nes;
assign ppumem_write = chr_write && (chr_allow || vram_ce) && !reset_nes;
