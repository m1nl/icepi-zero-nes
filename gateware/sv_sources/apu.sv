...
// 3-stage pipeline for output sample
always_ff @(posedge clk) begin
    squares   <= square1 + square2;
    squares_r <= squares;
    ch1       <= pulse_lut[squares];

    tri_out   <= 9'(tri_lut[triangle]);
    noise_out <= 9'(noise_lut[noise]);
    dmc_out   <= 9'(dmc_lut[dmc]);
