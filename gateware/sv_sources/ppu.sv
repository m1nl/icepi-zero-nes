...
wire new_is_in_vblank = entering_vblank ? 1 : exiting_vblank ? 1'b0 : is_in_vblank;
...
always @(posedge clk)
    if (reset) begin
	    ...
	    cycle <= end_of_line ? 0 : cycle + 9'd1;
	    ...
	    if (scanline == vsync_start_sl && hsync_period)
	    	vsync <= 1;
	    if (scanline == (vsync_start_sl + 3) && hsync_period)
	    	vsync <= 0;
...
wire using_secondary = rendering && ((~cycle[0] && cycle <= 255) || cycle > 255) ? 1 : 1'b0;
...
always @(posedge clk) begin :oam_eval
    ...
    if (reset) begin
	    ...
	    sprite0_curr     <= 0;
	    overflow         <= 0;
	    oam_secondary_column     <= 0;
	    ex_ovr           <= 0;
	    oam_addr_ex      <= 0;
	    oam_addr         <= 0;
	    oam_state <= STATE_IDLE;
	    old_rendering <= 0;
	    old_using_secondary <= 0;
    end else if (ce) begin
	    	...
	    	if (cycle < 64) begin               // Initialization state
	    		...
	    		if (evaluating || (visible && PAL)) begin
	    			...
	    			if (&spr_counter && ~ex_ovr) begin
	    				...
	    				if (scanline[7:0] >= oam[{oam_addr_ex, 2'b00}] &&
	    					...
	    					if (oam_secondary_row_ex < 8) begin // Turbo style.
	    						oam_secondary_row_ex <= oam_secondary_row_ex + 1;
	    			...
	    			if (!secondary_write) begin
	    				...
	    				if (eval_count == 0) begin // Evaluate Y for in_range
	    		...
	    		if ((cycle[2:0] < 3'd3 || cycle[2:0] == 3'd7) && ~oam_secondary_ovr)
	    			{oam_secondary_ovr, oam_secondary_addr} <= {1'b0, oam_secondary_addr} + 6'd1;
...
// Flip incoming vram data based on flipx. Zero out the sprite if it's invalid. The bits are already flipped once.
wire [7:0] vram_f =
	(dummy_sprite || masked_sprites || ~rendering) ? 0 :
	!flip_x ? {vram_data[0], vram_data[1], vram_data[2], vram_data[3], vram_data[4], vram_data[5], vram_data[6], vram_data[7]} :
	vram_data;
...
assign out = (bg_valid | obj_valid) ? (is_obj ? {1'b1, obj} : {1'b0, bg}) : 0;
...
// wire w2000 = cs_w && (ain == 0);
// wire w2001 = cs_w && (ain == 1);
// wire r2002 = cs_r && (ain == 2);
// wire r2003 = cs_r && (ain == 3);
// wire r2004 = cs_r && (ain == 4);
// wire w2004 = cs_w && (ain == 4);
// wire w2005 = cs_w && (ain == 5);
// wire w2006 = cs_w && (ain == 6);
// wire r2007 = cs_r && (ain == 7);
// wire w2007 = cs_w && (ain == 7);
...
always @(posedge clk) begin
	if (reset) begin
		obj_patt         <= 0; // 2000 resets to 0
		bg_patt          <= 0; // 2000 resets to 0
		obj_size         <= 0; // 2000 resets to 0
		vbl_enable       <= 0;
		grayscale        <= 0; // 2001 resets to 0
		playfield_clip   <= 0; // 2001 resets to 0
		object_clip      <= 0; // 2001 resets to 0
		enable_playfield <= 0; // 2001 resets to 0
		enable_objects   <= 0; // 2001 resets to 0
		emph_reg         <= 0; // 2001 resets to 0
		nmi_occured      <= 0; // wasn't reset before!
		re_sr            <= 0; // rendering disabled on reset
	end else if (ce) begin
...
always @(posedge clk) begin
	...
	if (ce) begin
		...
		if (decay_high > 0)
			decay_high <= decay_high - 1;
		else
			latched_dout[7:5] <= 3'b000;

		if (decay_low > 0)
			decay_low <= decay_low - 1;
		else
